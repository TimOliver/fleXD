...
/// supports being selected as indicated by `canSelectRow:` Subclasses
/// must implement this in accordance with how they implement `canSelectRow:`
/// if they do not implement `viewControllerToPushForRow:`
/// @return This returns `nil` if no view controller is provided by
/// `viewControllerToPushForRow:` — otherwise it pushes that view controller
...
/// Protected, not intended for public use. `menuTitleForRow:`
/// already includes the value returned from this method.
/// 
/// By default, this returns `@""` Subclasses may override to
...
/// By default, only includes items for `copyMenuItemsForRow:`
...
///
/// Custom reuse identifiers should be specified in `cellRegistrationMapping`
/// You may return any of the identifiers in `FLEXTableView.h`
/// without including them in the `cellRegistrationMapping`
/// @return `kFLEXDefaultCell` by default.

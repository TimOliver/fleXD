...
///
/// Example usage for a \c void method that takes an \c int:
/// @code FLEXTypeEncodingString(@encode(void), 1, @encode(int)); @endcode
/// @param returnType The encoded return type, e.g. \c @encode(void).
/// @param count The number of parameter type encodings that follow.
/// @return The full type encoding string, or \c nil if \e returnType is \c NULL.
...
/// Calls \c FLEXGetAllSubclasses.
@property (nonatomic, readonly, class) NSArray<Class> *flex_allSubclasses;

/// The metaclass of the receiving class, or \c Nil if the class is \c Nil or unregistered.
@property (nonatomic, readonly, class) Class flex_metaclass;

/// The size in bytes of instances of the receiving class, or \c 0 if the class is \c Nil.
@property (nonatomic, readonly, class) size_t flex_instanceSize;

/// Changes the class of the receiving object instance.
/// @return The previous class of the object, or \c Nil if the object is \c nil.
...
/// Calls \c FLEXGetConformedProtocols.
...
/// call this on \c [self superclass].
...
/// Returns the instance method with the given name, or \c nil if not found.
+ (nullable FLEXMethod *)flex_methodNamed:(NSString *)name;

/// Returns the class method with the given name, or \c nil if not found.
...
/// implementation, use \c replaceImplementationOfMethod:with:useInstance:.
///
/// @param typeEncoding The type encoding string. Consider using \c FLEXTypeEncodingString().
/// For example, for an \c NSArray \c count getter: \n
/// @code FLEXTypeEncodingString(@encode(NSUInteger), 0) @endcode
/// @param instanceMethod \c YES to add an instance method, \c NO to add a class method.
/// @return \c YES if the method was added, \c NO if one with that selector already exists.
...
///   \c addMethod:typeEncoding:implementation:toInstances: were called.
/// - If the method already exists, its \c IMP is replaced.
///
/// @param instanceMethod \c YES to replace the instance method, \c NO for the class method.
/// @return The previous \c IMP of the method.
...
/// \c FLEXMethodBase contains a valid selector.
/// @param instanceMethod \c YES to swizzle instance methods, \c NO for class methods.
...
/// @param instanceMethod \c YES for instance methods, \c NO for class methods.
/// @return \c YES if successful, \c NO if the selectors could not be resolved.
...
/// Returns the instance variable with the given name, or \c nil if not found.
+ (nullable FLEXIvar *)flex_ivarNamed:(NSString *)name;

/// Returns the address of the given ivar in the receiving object, or \c NULL if not found.
- (nullable void *)flex_getIvarAddress:(FLEXIvar *)ivar;

/// Returns the address of the named ivar in the receiving object, or \c NULL if not found.
- (nullable void *)flex_getIvarAddressByName:(NSString *)name;

/// Returns the address of the given \c Ivar in the receiving object, or \c NULL if not found.
/// Faster than \c flex_getIvarAddress: when you already have the raw \c Ivar.
...
/// @discussion Use only when the target ivar holds an object.
/// @return \c YES if successful, \c NO if the ivar was not found.
- (BOOL)flex_setIvarByName:(NSString *)name object:(id)value;

/// Sets the value of the given \c Ivar on the receiving object.
/// @discussion Use only when the target ivar holds an object.
/// Faster than \c flex_setIvar:object: when you already have the raw \c Ivar.
...
/// Sets the value of the given \c Ivar on the receiving object to the \e size bytes at \e value.
/// Faster than \c flex_setIvar:value:size: when you already have the raw \c Ivar.
...
/// Returns the instance property with the given name, or \c nil if not found.
+ (nullable FLEXProperty *)flex_propertyNamed:(NSString *)name;
/// Returns the class property with the given name, or \c nil if not found.
